...
class JobOrderTicketSummary(TimeStamped):
    ticket_summary_job_order = models.ForeignKey(
        "post_paid.TicketSummary",
        related_name="ticket_summary_job_orders",
    ...
    client = models.ForeignKey(
        "authentication.Client",
        related_name="client_ticket_summary_job_orders",
        on_delete=models.PROTECT,
        blank=True,
        null=True,
    )
    ...
    client_email = models.EmailField(blank=True)
    va_assigned = models.ManyToManyField(
        "authentication.Staff", related_name="vas_ticket_summary_job_orders", blank=True
    )
    staff_email = models.CharField(max_length=500, blank=True)
    ticket_number = models.CharField(max_length=100, blank=True)
    request_date = models.DateField()
    ...
    class Meta:
        ordering = ["-ticket_number"]
        verbose_name = "List of Ticket Summary Job Order"
        verbose_name_plural = "List of ticket Summary Job Orders"
    ...
    def create_ticket_number(self):
        ticket_code = ""
        last_in = JobOrderTicketSummary.objects.all().order_by("id").last()
    ...
    def get_company_client(self):
        if self.ticket_summary_job_order:
            client = self.ticket_summary_job_order.company.client
            return client
        elif self.client:
            return self.client
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            self.client = self.get_company_client()
            self.client_file = self.get_account_files()
            super(JobOrderTicketSummary, self).save(*args, **kwargs)
        elif self.id:
            self.ticket_number = self.create_ticket_number()
            self.client = self.get_company_client()
            self.client_file = self.get_account_files()
...
class JobOrderTicketSummaryComment(TimeStamped):
    job_order = models.ForeignKey(
        JobOrderTicketSummary,
        related_name="ticket_summary_job_order_comments",
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
    )

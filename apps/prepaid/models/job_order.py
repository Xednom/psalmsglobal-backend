...
__all__ = ("JobOrderPrepaid", "JobOrderCommentPrepaid")
...
class JobOrderPrepaid(TimeStamped):
    ...
    client = models.ForeignKey(
        ...
        related_name="prepaid_client_job_orders",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    ...
    va_assigned = models.ManyToManyField(
        "authentication.Staff", related_name="prepaid_va_job_orders", blank=True
    )
    ...
    def create_ticket_number(self):
        ticket_code = ""
        last_in = JobOrderPrepaid.objects.all().order_by("id").last()
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            super(JobOrderPrepaid, self).save(*args, **kwargs)
        elif self.id:
            self.ticket_number = self.create_ticket_number()
            self.client = self.get_company_client()
            self.client_file = self.get_account_files()
...
class JobOrderCommentPrepaid(TimeStamped):
    job_order = models.ForeignKey(
        JobOrderPrepaid,
        related_name="prepaid_job_order_comments",
